...
- Color-coded: hospitals (red), government/public buildings (blue), Mercer University (orange), and named landmarks (brass) always take priority; every other building is colored by its actual zoning parcel from Bibb County's tax assessor data (residential/commercial/industrial/agricultural/other), not left flat tan — see `scripts/fetch_zoning.py`/`build_colored_disk.py`'s `classify_zoning()`. Roads (grey), parking lots (lavender-grey), water (blue), terrain (green), and real airport pavement (runways, taxiways, aircraft aprons) at Middle Georgia Regional and Macon Downtown airports.
- Every legend category (each building type, roads, parking, water, terrain, place labels) can be shown/hidden independently — click a row in the side panel, grouped into sections (Special-cased buildings, zoning, Infrastructure, Airport, Terrain, plus the historical/demographic overlays) each with its own select-all/none master toggle. The model is exported as a multi-node scene specifically so this works (see `CLAUDE.md`). Building categories also have a second, independent toggle: click a category's color swatch to grey it out ("un-highlight") without hiding it. A separate toggle under Terrain switches to true-to-scale elevation (no 2.5x vertical exaggeration), for comparing the model's dramatized relief against Macon's real, flatter terrain.
...
- Labeled: highway routes (I-75, US 41, ...) as distinct shield badges, named rivers and lakes, and ~155 named neighborhoods, real industrial employers, and smaller communities (from OpenStreetMap, filtered to real numbered routes and building-dense named places so the map isn't overlabeled), all floating above the terrain and fading in by camera distance — highways/rivers/industrial employers visible from farthest away, neighborhoods at medium range, smaller communities only up close — see `scripts/fetch_places.py`/`build_labels.py`. Landmarks go further: a hand-curated set of medical facilities, notable buildings, notable churches, Macon's music heritage (Little Richard, Otis Redding, Capricorn Records, the Big House, Rose Hill Cemetery, the Douglass Theatre), and Ocmulgee Mounds National Historical Park, each category with its own marker color — see `site/assemble.py`'s `LANDMARKS`.
...
- Click any landmark in the side panel or on the model to smoothly fly the camera to it. Two search boxes in the panel: one filters the legend itself (find a specific layer toggle by name), the other searches every landmark, neighborhood, road, and river by name and flies straight to it.
...
- Embeddable in an `<iframe>` (e.g. on a personal website) without stepping on the host page: an embedded visitor gets a slow ambient auto-orbit preview instead of live WASD/scroll-wheel controls, with a "click to explore" gate that hands off to fullscreen, real interactive mode, and the normal onboarding once clicked — no separate build, this is automatic (`window.top !== window.self`) and has zero effect on a direct visit. The embedding `<iframe>` tag needs `allowfullscreen` for the fullscreen handoff to actually work; it'll still enter interactive mode without it, just not fullscreen.

## Where this project is headed

This repo shares its data-prep pipeline with the sibling **macon-3d-print** repo (the physically-printable version), but the two are expected to diverge substantially. Now covers the whole county (clipped to the real county polygon, not the circle this started as) instead of just downtown; next up is likely more data overlays (commissioner district boundaries, school districts, flood zones — see `docs/OVERLAYS.md`) and/or even better building data (Overture Maps Foundation, Bibb County's own GIS). See `CLAUDE.md` for the current architecture and the reasoning behind it.
